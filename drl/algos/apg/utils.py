...
def discount_return(reward, done, bootstrap_value, eta, discount, return_dest=None):
    ...
    return_[-1] = nd[-1] * (reward[-1] - eta + discount * bootstrap_value)
    for t in reversed(range(len(reward) - 1)):
        return_[t] = nd[t] * (reward[t] - eta + discount * return_[t + 1])
    return return_
...
def generalized_advantage_estimation(reward,
    ...
    nd = 1 - done
    nd = nd.type(reward.dtype) if isinstance(nd, torch.Tensor) else nd
    advantage[-1] = nd[-1] * (reward[-1] - eta + discount * bootstrap_value - value[-1])
    for t in reversed(range(len(reward) - 1)):
        delta = nd[t] * (reward[t] - eta + discount * value[t + 1] - value[t])
