...
class RiskNonCrossZofMuMlpModel(torch.nn.Module):
    ...
    def __init__(
        ...
        # entropy
        self.interval_head_h = nn.Sequential(nn.Linear(last_size, embedding_size), nn.Softmax(-1))
        self.scale_head_h = nn.Sequential(nn.Linear(last_size, 1), nn.Softplus())
        self.position_head_h = nn.Linear(last_size, 1)
    ...
    def forward(self, observation, prev_action, prev_reward, action, tau):
        ...
        h = self.get_dist(
            position=self.position_head_h(hidden),  # (N, 1)
            scale=self.scale_head_h(hidden),  # (N, 1)
            intervals=self.interval_head_h(hidden),  # (N, E)
            tau=tau,
            embedding_size=self.embedding_size)
    ...
    @staticmethod
    def get_dist(position, scale, intervals, tau, embedding_size=64):
        values = torch.cat([position, intervals.cumsum(-1) * scale + position], -1)  # (N, E+1)
        i_l = torch.floor(tau * embedding_size).clamp(min=0, max=embedding_size)  # (N, M)
        i_r = torch.ceil(tau * embedding_size).clamp(min=0, max=embedding_size)  # (N, M)
        z_l = values.gather(-1, i_l.long())  # (N, T)
        z_r = values.gather(-1, i_r.long())  # (N, T)
        # if i_r > i_l: i_r = i_l + 1, z_r > z_l
        # else: i_r = i_l, z_r = z_l
        z = z_l + (tau * embedding_size - i_l) * (z_r - z_l)
        return z
